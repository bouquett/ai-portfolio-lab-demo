...
function calendarDates(start, end) {
  const dates = [];
  for (let value = Date.parse(`${start}T00:00:00Z`); value <= Date.parse(`${end}T00:00:00Z`); value += DAY_MS) {
...
export async function loadAssetHistory(asset, years = 10) {
  const end = isoDate(Date.now());
  const start = addDays(end, -Math.ceil(Number(years) * 365.25) - 45);
  if (asset.type === "现金") {
    return buildCashSeries(calendarDates(start, end), asset.cashRate ?? 0.015, asset.code, asset.name);
  }
  try {
    return await fetchAssetHistory(asset, start, end);
  } catch (error) {

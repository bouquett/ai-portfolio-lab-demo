...
export function allocationConstraintsMet(weights, categories) {
  ...
  if (Math.abs(weights.reduce((sum, weight) => sum + weight, 0) - 1) > 1e-7) return false;
  if (Math.max(...weights) > 0.35000001) return false;
  ...
  if (totals.has("现金") && Math.abs(totals.get("现金") - 0.05) > 1e-7) return false;
  if (totals.has("美股") && Math.abs(totals.get("美股") - 0.1) > 1e-7) return false;
  if (totals.has("固收") && (totals.get("固收") < (fixedPositions.length >= 3 ? 0.35 : 0.25) || totals.get("固收") > (fixedPositions.length >= 3 ? 0.55 : 0.6))) return false;
  if (fixedPositions.length >= 3 && fixedPositions.some((index) => weights[index] < 0.05 || weights[index] > 0.2)) return false;
  if (fixedPositions.length >= 3 && fixedPositions.some((index) => weights[index] / totals.get("固收") > 0.45)) return false;
  if (totals.has("A股宽基") && (totals.get("A股宽基") < (broadPositions.length >= 2 ? 0.2 : 0.1) || totals.get("A股宽基") > (broadPositions.length >= 2 ? 0.4 : 0.5))) return false;
  if (broadPositions.length >= 2 && broadPositions.some((index) => weights[index] < 0.05 || weights[index] > 0.25)) return false;
  if (totals.has("黄金") && (totals.get("黄金") < 0.05 || totals.get("黄金") > 0.2)) return false;
  if ((totals.get("其他") ?? 0) > 0.25) return false;
  return true;
}
...
function structuredDiversifiedSample(categories, random) {
  ...
  if (!positions["现金"].length || !positions["美股"].length || positions["固收"].length < 3 || positions["A股宽基"].length < 2 || !positions["黄金"].length) return null;

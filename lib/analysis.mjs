...
export function analyzePortfolioData(rawAssets, prices, options = {}) {
  ...
  const broadIndexAssets = assets.filter((asset) => ["A股宽基", "美股"].includes(asset.category));
  ...
  if (optimizedGoldWeight > 0.15) warnings.push(`历史候选的黄金权重为 ${(optimizedGoldWeight * 100).toFixed(1)}%，超过稳态方案的 10%；这可能反映样本期黄金表现强势，需防止追涨和历史过拟合。`);
